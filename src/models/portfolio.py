...
class Portfolio(BaseModel):
    ...
    education: List[EducationItem] = Field(default_factory=list, description="교육 이력 목록")
    ...
    experience: List[Dict[str, Any]] = Field(default_factory=list, description="경력 사항")
    projects: List[Dict[str, Any]] = Field(default_factory=list, description="프로젝트 이력")
    ...
    created_at: Optional[datetime] = Field(default_factory=datetime.now, description="생성일시")
    updated_at: Optional[datetime] = Field(default_factory=datetime.now, description="수정일시")
    ...
    def generate_document_id(self) -> str:
        ...
        if not self.document_id:
            self.document_id = f"portfolio_{self.user_id}_{int(self.created_at.timestamp())}"
    ...
    def to_chroma_metadata(self) -> Dict[str, Any]:
        ...
        metadata = {
            ...
            "created_timestamp": int(self.created_at.timestamp()) if self.created_at else 0,
            "updated_timestamp": int(self.updated_at.timestamp()) if self.updated_at else 0
        }
...
def create_portfolio_from_basic_info(user_id: int, user_name: str, age: int, 
                                   collection_name: str = "portfolios") -> Portfolio:
